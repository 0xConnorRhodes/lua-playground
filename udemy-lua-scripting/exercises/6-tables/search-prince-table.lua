...
function search_table(page, line, word)
  local code_found = false
  for _, table in ipairs(prince_codes) do
    if (table.Page == page and
        table.Line == line and
        table.Word == word) then
          code_found = true
          return 'The code is '..table.Code
    end
  end

  if not code_found then
    return 'That code is not in the database.'
  end
end

local search = search_table(page_input, line_input, word_input)
print(search)